...
光流统计已接入解释证据契约，并通过服务器上的 4 次真实 API 复验，尚未达到 Overview 的完整临床语义解释。该缺口需通过临床定义、
...
| Explanation | API 事实排序与严格核验，D 证据在线验证通过 | 真实 Y 模型、临床语言与医生盲评 |
...
原始图像、人工标注、逐病例运行目录、模型权重和 .env 排除在 Git 之外，保留在服务器。
按 2026-09-20 最新要求，本地只保留代码、配置、测试与技术文档；历史实验备份已核验后转存服务器。
